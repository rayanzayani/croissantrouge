...
// Add plugin
require_once 'Tpl/PathReplace.php';
Template::registerPlugin(new PathReplace());
...
class Template {
    ...
    protected function checkTemplate($template) {
        ...
        $parsedTemplateFilepath = static::$conf['cache_dir'] . $templateName . '.ctpl.php';

        // if the template doesn't exsist throw an error
		
        if (!file_exists($templateFilepath)) {
            //$e = new NotFoundException('Template ' . $templateName . ' not found! ' . $templateFilepath);
    ...
    protected function compileTemplate($code, $isString, $templateBasedir, $templateDirectory, $templateFilepath) {
        ...
        //read all parsed code
            foreach ($codeSplit as $html) {
                ...
                //constants
                elseif (preg_match($tagMatch['block'], $html, $matches)) {
				$urlArray = explode("_", $matches[1]);
				...
				$dispatch = new $controller($bundle, $controllerName, $action);
				$action =  'block_'.$action;
				$block = call_user_func_array(array($dispatch, $action), array());
