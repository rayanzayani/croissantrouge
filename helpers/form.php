...
class Form {
	...
	public function __construct($uniqid, $method = 'get') {
		...
		if (false !== $uniqid && in_array($uniqid, self::$instances)) {
			...
			trigger_error("Un formulaire identifi̩ par '$uniqid' existe d̩ja  ! Conflits potentiels d̩tect̩s !", E_USER_WARNING);
...
// ---------------------------------------------------------------------------
class Form_Div extends Form_Field {
	...
	public function __toString() {
		...
		return '<div'.$this->attrs.'>'.$this->value.'</div>';
	}
}
// ---------------------------------------------------------------------------
...
class Form_Submit extends Form_Input {
	...
	public function __toString() {
		...
		$field = '<input class="small button blue"'.$this->attrs.$value.' />';
